...
#if defined(NDEBUG)
# error "Agecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1b8a542153c15baa8180362ec94f0463da25907d6e7513c478584bcb20b975c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Agecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Agecoin Signed Message:\n";
...
{
    // Agecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Agecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Agecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1455865686, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0412304768586e97c4a8439c74bb1a523d7adace001861d3852efb71ad44e95da5fff5d1e6c023b5ac4425bee93e4302c8dc390bf36910f9dd0ca23f5531640f93") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1455865686;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5538201;
...
unsigned char pchMessageStart[4] = { 0xbe, 0x18, 0xe9, 0x1d }; // Agecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AgecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AgecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AgecoinMiner(CWallet *pwallet)
{
    printf("AgecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("agecoin-miner");
    ...
    try { loop {
        ...
        printf("Running AgecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AgecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AgecoinMiner, pwallet));
}
